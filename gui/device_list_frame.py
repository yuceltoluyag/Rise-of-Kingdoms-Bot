...
class DeviceListFrame(Frame):
    ...
    def __init__(self, notebook, main_frame, cnf={}, **kwargs):
        ...
        for config in self.devices_config:
            dlt.add_row(
                config.get("name", "None"), config["ip"], config["port"]
            )
...
class DeviceRow(Frame):
    def __init__(
        self, device_list_table, main_frame, name, ip, port, cnf={}, **kwargs
    ):
    ...
    def set_on_display_click(self, on_click=lambda self: self):
        ...
        def callback():
            ...
            device.save_file_prefix = (
                f"{self.name}_{device.serial.replace(':', '_')}"
            )
            if device is None:
                return
...
class AddDeviceFrame(Frame):
    ...
    def __init__(self, parent, cnf={}, **kwargs):
        ...
        self.ip_entry.config(
            width=15,
            validate="key",
            validatecommand=(self.register(ip_entry_validate_cmd), "%P", "%d"),
        )
        self.port_entry.config(
            width=8,
            validate="key",
            validatecommand=(
                self.register(port_entry_validate_cmd),
                "%P",
                "%d",
            ),
        )
