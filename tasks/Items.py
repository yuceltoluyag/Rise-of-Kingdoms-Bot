...
class Items(Task):
    ...
    def use_vip(self):
        ...
        self.tap(x, y, 0.5)
        for item in [
            ImagePathAndProps.ITEM_VIP1_IMAGE_PATH,
            ImagePathAndProps.ITEM_VIP2_IMAGE_PATH,
        ]:
            found = True
            while found:
    ...
    def use_gems(self):
        ...
        self.tap(x, y, 0.5)
        for item in [
            ImagePathAndProps.ITEM_GEMS1_IMAGE_PATH,
            ImagePathAndProps.ITEM_GEMS2_IMAGE_PATH,
    ...
    def use_daily_rss(self):
        if not self.bot.config.useItemsDailyRss:
            ...
            return
        self.set_text(
            insert="Opening daily quest resource items (lvl 1 resource pack only)"
        )
        # Reopen Items Window so that item values are reset to 1
        self.back_to_home_gui()
        self.menu_should_open(True)
        ...
        for i in range(0, 5):
            found, _, pos = self.gui.check_any(
                ImagePathAndProps.ITEM_RESOURCE_PACK1_IMAGE_PATH.value
            )
            if not found:
                return
            ...
            self.tap(x, y, 1)
            found, _, pos = self.gui.check_any(
                ImagePathAndProps.ITEM_EXCESS_RESOURCE_PROMPT_NO_IMAGE_PATH.value
            )
            if found:
