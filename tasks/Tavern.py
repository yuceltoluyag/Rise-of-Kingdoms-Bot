...
class Tavern(Task):
    ...
    def do(self, next_task=TaskName.TRAINING):
        ...
        super().tap(x, y, 1)
        _, _, tavern_btn_pos = self.gui.check_any(
            ImagePathAndProps.TAVERN_BUTTON_BUTTON_IMAGE_PATH.value
        )
        ...
        super().tap(x, y, 4)
        for i in range(20):
            _, _, open_btn_pos = self.gui.check_any(
                ImagePathAndProps.CHEST_OPEN_BUTTON_IMAGE_PATH.value
            )
            ...
            super().tap(x, y, 4)
            _, _, confirm_btn_pos = self.gui.check_any(
                ImagePathAndProps.CHEST_CONFIRM_BUTTON_IMAGE_PATH.value
            )
